#!/usr/bin/env python3
# 🧮 بوت اختبارات رياضيات النهايات - النسخة البسيطة والمستقرة
...
def home():
    ...
    <html>
        ...
        <body>
            ...
            <p>📱 للطلاب: اكتب /start في البوت</p>
        </body>
    </html>
    """
...
class Database:
    ...
    def add_shown_question(self, user_id, q_type, q_id):
        """تسجيل السؤال المعروض للطالب"""
        user_id = str(user_id)
        if user_id in self.data['students']:
            if q_id not in self.data['students'][user_id]['questions_shown'][q_type]:
                self.data['students'][user_id]['questions_shown'][q_type].append(q_id)
                # الحفاظ على آخر 10 أسئلة فقط لتجنب التراكم
                if len(self.data['students'][user_id]['questions_shown'][q_type]) > 10:
                    self.data['students'][user_id]['questions_shown'][q_type] = \
                        self.data['students'][user_id]['questions_shown'][q_type][-10:]
                self.save_data()
    
    def get_available_questions(self, user_id, q_type, all_questions):
        """الحصول على الأسئلة المتاحة (غير المعروضة للطالب)"""
        user_id = str(user_id)
        if user_id not in self.data['students']:
            return all_questions
        
        shown_ids = self.data['students'][user_id]['questions_shown'][q_type]
        available = [q for q in all_questions if q['id'] not in shown_ids]
        
        # إذا لم يتبق أسئلة متاحة، نعيد تعيين القائمة
        if not available and shown_ids:
            self.data['students'][user_id]['questions_shown'][q_type] = []
            self.save_data()
            return all_questions
...
# 📚 الأسئلة (جميع الأسئلة من الملفين)
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        ...
        # إعادة تعيين الأسئلة المعروضة في كل بداية جلسة
        db.data['students'][str(user.id)]['questions_shown'] = {'tf': [], 'mcq': []}
        db.save_data()
...
async def truefalse_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الحصول على الأسئلة المتاحة (غير المعروضة سابقاً)
    available_questions = db.get_available_questions(user_id, 'tf', TRUE_FALSE_QUESTIONS)
    
    if not available_questions:
        await update.message.reply_text("🎯 لقد عرضت جميع أسئلة الصح/خطأ! استخدم /start لبدء جلسة جديدة.")
        return
    
    q = random.choice(available_questions)
    
    # تسجيل السؤال كمعروض
    db.add_shown_question(user_id, 'tf', q['id'])
    ...
    remaining = len(TRUE_FALSE_QUESTIONS) - len(db.data['students'].get(str(user_id), {}).get('questions_shown', {}).get('tf', []))
    text = f"🔵 سؤال صح/خطأ ({remaining} سؤال متبقٍ)\n\n❓ {q['q']}"
...
async def mcq_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # الحصول على الأسئلة المتاحة (غير المعروضة سابقاً)
    available_questions = db.get_available_questions(user_id, 'mcq', MCQ_QUESTIONS)
    
    if not available_questions:
        await update.message.reply_text("🎯 لقد عرضت جميع أسئلة الخيارات! استخدم /start لبدء جلسة جديدة.")
        return
    
    q = random.choice(available_questions)
    
    # تسجيل السؤال كمعروض
    db.add_shown_question(user_id, 'mcq', q['id'])
    ...
    remaining = len(MCQ_QUESTIONS) - len(db.data['students'].get(str(user_id), {}).get('questions_shown', {}).get('mcq', []))
    text = f"🔴 سؤال خيارات ({remaining} سؤال متبقٍ)\n\n❓ {q['q']}"
...
async def handle_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    shown_tf = len(db.data['students'][user_id]['questions_shown']['tf'])
    shown_mcq = len(db.data['students'][user_id]['questions_shown']['mcq'])
    remaining_tf = len(TRUE_FALSE_QUESTIONS) - shown_tf
    remaining_mcq = len(MCQ_QUESTIONS) - shown_mcq
    
    msg += f"\n\n📚 الأسئلة المتبقية:\nصح/خطأ: {remaining_tf} سؤال\nخيارات: {remaining_mcq} سؤال"
    msg += "\n\n🔁 /truefalse - /mcq"
...
async def score_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # حساب الأسئلة المعروضة والمتبقية
    shown_tf = len(student['questions_shown']['tf'])
    shown_mcq = len(student['questions_shown']['mcq'])
    remaining_tf = len(TRUE_FALSE_QUESTIONS) - shown_tf
    remaining_mcq = len(MCQ_QUESTIONS) - shown_mcq
    
    report = f"📊 نتيجتك:\n✅ {correct} صحيح\n❌ {total-correct} خطأ\n🎯 {percent:.1f}%\n📅 {student['joined']}"
    report += f"\n\n📚 تقدمك:\nصح/خطأ: {shown_tf}/{len(TRUE_FALSE_QUESTIONS)} ({remaining_tf} متبقٍ)"
    report += f"\nخيارات: {shown_mcq}/{len(MCQ_QUESTIONS)} ({remaining_mcq} متبقٍ)"
    
    await update.message.reply_text(report)

async def reset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """إعادة تعيين الأسئلة المعروضة"""
    user_id = str(update.effective_user.id)
    if user_id in db.data['students']:
        db.data['students'][user_id]['questions_shown'] = {'tf': [], 'mcq': []}
        db.save_data()
        await update.message.reply_text("🔄 تم إعادة تعيين الأسئلة المعروضة! يمكنك الآن رؤية جميع الأسئلة من جديد.")
    else:
        await update.message.reply_text("⚠️ اكتب /start أولاً")

# 🔧 تشغيل البوت بطريقة أبسط
def run_bot():
    """تشغيل البوت بطريقة تمنع أي conflicts"""
    print("=" * 50)
    ...
    async def main():
        try:
            # 1. أولاً: تأكد من إغلاق أي اتصال سابق
            ...
            try:
                ...
                print("✅ تم حذف أي webhook سابق")
            except:
                pass
            
            # 2. إنشاء التطبيق مع خيار drop_pending_updates
            ...
            # 4. البدء مع تنظيف التحديثات القديمة
            await application.initialize()
            await application.start()
            print("✅ تم تهيئة البوت")
            
            # 5. بدء الـ polling مع تنظيف كامل للتحديثات السابقة
...
def main():
    ...
    # خيار 1: تشغيل Flask في خيط منفصل
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    
    # الانتظار قليلاً قبل تشغيل البوت
    time.sleep(3)
